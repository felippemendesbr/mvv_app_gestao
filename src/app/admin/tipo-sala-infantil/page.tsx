...
export default function TipoSalaInfantilPage() {
  ...
  async function fetchTipos() {
    try {
      ...
      if (!res.ok) throw new Error("Erro ao carregar tipos de sala infantil");
  ...
  async function handleDelete(id: number) {
    if (!confirm("Deseja realmente excluir este tipo de sala infantil?"))
      return;
    try {
  ...
  const columns: Column<TipoSalaInfantil>[] = [
    {
      key: "id",
      label: "ID",
      sortable: true,
    ...
    },
    {
      ...
      render: (t) => (
        <div className="flex gap-2">
          <Link
            href={`/admin/tipo-sala-infantil/${t.id}`}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Baby className="h-8 w-8 text-[var(--secondary)] dark:text-[#D7C7A3]" />
          <div>
            <h1 className="text-3xl font-bold text-[var(--foreground)]">
              Tipos de Sala Infantil
            </h1>
            <p className="text-[var(--foreground)]/80 mt-1">
              Gerencie os tipos de sala infantil
            </p>
          </div>
        </div>
        <Link href="/admin/tipo-sala-infantil/novo">
          <Button>
      ...
      <Card>
        <div className="p-6">
          <DataGridPaginated
            ...
            columns={columns}
            getRowId={(t) => t.id}
            emptyMessage="Nenhum tipo de sala infantil cadastrado"
            exportFileName="tipos-sala-infantil"
          />
        </div>
      </Card>
    </div>
