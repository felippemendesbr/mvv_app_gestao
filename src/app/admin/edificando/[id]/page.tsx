...
export default function EditarEdificandoPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`/api/edificando/${id}`);
        if (!res.ok) throw new Error("Registro não encontrado");
        const data = await res.json();
        setTitulo(data.titulo);
        setPdfs(
          data.pdfs?.length > 0
            ? data.pdfs.map((p: { titulo: string; url: string }) => ({
                titulo: p.titulo,
                url: p.url,
              }))
            : [{ titulo: "", url: "" }]
        ...
        setVideos(
          data.videos?.length > 0
            ? data.videos.map((v: { titulo: string; url: string }) => ({
                titulo: v.titulo,
                url: v.url,
              }))
            : [{ titulo: "", url: "" }]
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-[var(--foreground)]">Editar Edição</h1>
          <p className="text-[var(--foreground)]/80 mt-1">Atualize título, PDFs e vídeos</p>
        </div>
      </div>
